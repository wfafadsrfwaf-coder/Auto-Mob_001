...
local function waitForMapToLoad(timeout)
    ...
    warn("แมพโหลดไม่ครบภายใน", timeout, "วินาที")
    return false
end
    ...
    task.spawn(function()
        while Character and HRP and Humanoid and Humanoid.Health > 0 do
            if HRP.Position.Y < -50 then
                warn("⚠️ ตรวจเจอตกแมพ รีเซ็ตตัวละครใหม่...")
                Humanoid.Health = 0
                break
            end
            task.wait(1)
...
local function teleportBackToFarmWorld()
    ...
    print("🚪 กำลังวาร์ปกลับไปโลกฟาร์มผ่าน Remote...")
    pcall(function()
...
local function teleportToInstance(inst)
    ...
    local char = Player.Character
    if char and char:IsA("Model") and HRP then
        local finalCF = targetCF + Vector3.new(0, 5, 0)
...
local function runContinuousFarm()
    ...
    if not remote then
        warn("❌ ไม่พบ Remotes.PlaySoundRemote ใน Dragons[1..50]")
        return
    end

    print("🚀 เริ่มฟาร์มแบบไม่หยุด...")
...
local function startResourceTracker()
    ...
    local function updateLabel()
        ...
        if not isTeleporting and (meatVal >= 10000 and baconVal >= 10000) then
            isTeleporting = true
            warn("📦 Meat และ Bacon ครบ 500 แล้ว! กำลังวาร์ปกลับ...")
...
task.spawn(function()
    if AUTO_FARM then
        ...
        if placeId == 4869039553 then
            print("🦖 เข้าสู่โลกฟาร์ม... รอโหลดและดีเลย์ก่อนเริ่มฟาร์ม")
            task.wait(8)
            startResourceTracker()
            runContinuousFarm()

        elseif placeId == 3475397644 then
            print("💰 อยู่โลกขายของ กำลังขายทั้งหมด...")
            sellAllItems()
            task.wait(2)
            teleportBackToFarmWorld()

        else
            warn("🌍 ไม่รู้จักโลกนี้: " .. tostring(placeId))
        end
    end
end) 
